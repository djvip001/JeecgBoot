<template>
  <BasicModal v-bind="$attrs" @register="registerModal" destroyOnClose :title="title" :width="896" @ok="handleSubmit">
    <BasicForm @register="registerForm" ref="formRef" name="UporderOrderForm"/>
    <!-- 子表单区域 -->
...
<script lang="ts" setup>
    ...
    import {formSchema,uporderOrderTrafficNoColumns} from '../UporderOrder.data';
    ...
    const [registerModal, {setModalProps, closeModal}] = useModalInner(async (data) => {
        ...
        if (unref(isUpdate)) {
            ...
            await setFieldsValue({
             ...
             requestSubTableData(uporderOrderTrafficNoList, {id:data?.record?.id}, uporderOrderTrafficNoTable)
        }
        // 隐藏底部时禁用整个表单
    ...
    async function reset(){
      await resetFields();
      activeKey.value = 'uporderOrderTrafficNo';
      uporderOrderTrafficNoTable.dataSource = [];
    }
